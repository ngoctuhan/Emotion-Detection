...
class PersonGrpcClient:
    ...
    __instance = None 
    @staticmethod
    def getInstance(ip, port, client_id="app"):
        if PersonGrpcClient.__instance is None:
            PersonGrpcClient.__instance = PersonGrpcClient(ip, port, client_id)
        return PersonGrpcClient.__instance
